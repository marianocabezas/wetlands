...
class Segmenter(BaseModel):
    ...
    def _cross_entropy(self, predicted, target):
        try:
            target, roi = target
            roi_channels = roi.unsqueeze(1).repeat(1, target.shape[1], 1, 1)
            predicted = predicted[roi_channels]
            target = target[roi]
        except ValueError:
            pass
...
class LRASPP_MobileNet(Segmenter):
    def __init__(
        ...
        self.last_features = self.lraspp.classifier.high_classifier.in_channels
        self.lraspp.classifier.high_classifier = nn.Conv2d(
            self.last_features, n_outputs, kernel_size=1, stride=1
        )
        self.last_features = self.lraspp.classifier.low_classifier.in_channels
